#include "stdafx.h"
#include "SFML\Window\Joystick.hpp"
#include "P2Controls.h"

P2Controls::P2Controls() :
m_DEADZONEA(15),
m_DEADZONEB(50),
m_counter(0),
m_isControllerConnected(true),
m_buttonCount(sf::Joystick::getButtonCount(0))
{
	m_buttons = std::vector<bool>();

	for (int i = 0; i < 10; i++)
	{
		m_buttons.push_back(false);
		m_buttonsPrev.push_back(m_buttons.at(i));
	}
}

sf::Vector2f P2Controls::GetLeftStickAxis()
{
	return m_leftAnalogStick;
}

sf::Vector2f P2Controls::GetRightStickAxis()
{
	return m_rightAnalogStick;
}

P2Controls& P2Controls::Instance()
{
	static P2Controls instance = P2Controls();
	return instance;
}

void P2Controls::Update()
{
	if (sf::Joystick::isConnected(1) == false)
		m_isControllerConnected = false;
	else
		m_isControllerConnected = true;


	m_leftAnalogStick = sf::Vector2f(0, 0);
	m_rightAnalogStick = sf::Vector2f(0, 0);

	m_leftAnalogAngle = 0;
	m_rightAnalogAngle = 0;

	int count = sf::Joystick::getButtonCount(0);
	for (int i = 0; i < sf::Joystick::getButtonCount(0); i++)
	{
		m_buttonsPrev.at(i) = m_buttons.at(i);

		if (sf::Joystick::isButtonPressed(1, i))
			m_buttons.at(i) = true;
		else 
			m_buttons.at(i) = false;
	}

	m_leftAnalogStickPrev = m_leftAnalogStick;

	m_leftAnalogStick = sf::Vector2f(sf::Joystick::getAxisPosition(1, sf::Joystick::X), sf::Joystick::getAxisPosition(1, sf::Joystick::Y));
	m_rightAnalogStick = sf::Vector2f(sf::Joystick::getAxisPosition(1, sf::Joystick::U), sf::Joystick::getAxisPosition(1, sf::Joystick::R));

	// Updates Left Analog Stick
	////////////////////////////////
	if (m_leftAnalogStick.x > m_DEADZONEA || m_leftAnalogStick.x < -m_DEADZONEA || m_leftAnalogStick.y > m_DEADZONEA || m_leftAnalogStick.y < -m_DEADZONEA)
	{
		m_leftStickEnabled = true;
		m_leftAnalogAngle = atan2(m_leftAnalogStick.y, m_leftAnalogStick.x);
	}
	else
		m_leftStickEnabled = false;

	// Updates Right Analog Stick
	////////////////////////////////
	if (m_rightAnalogStick.x > m_DEADZONEB || m_rightAnalogStick.x < -m_DEADZONEB || m_rightAnalogStick.y > m_DEADZONEB || m_rightAnalogStick.y < -m_DEADZONEB)
	{
		m_rightStickEnabled = true;
		m_rightAnalogAngle = atan2(m_rightAnalogStick.y, m_rightAnalogStick.x);
	}
	else
		m_rightStickEnabled = false;
}

bool P2Controls::CheckIfControllerIsConnected()
{
	return m_isControllerConnected;
}
